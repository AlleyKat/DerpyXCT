...
--some init
local ct,M,L = unpack(select(2,...))
local blank = M['media'].blank
ct.damagefont = M['media'].font
ct.font = M['media'].font
ct.critprefix = "|cffFF0000*|r"
ct.critpostfix = "|cffFF0000*|r"
ct.myclass = M.class
---------------------------------------------------------------------------------
...
local on_push = function(self,button)
	local a = "justify_"..self.num
	if button == "RightButton" then
		HissyXCT_VARS[a] = help_sw_r[HissyXCT_VARS[a]]
	else
		HissyXCT_VARS[a] = help_sw_l[HissyXCT_VARS[a]]
	end
	self.m:SetText(HissyXCT_VARS[a])
end

local onenter = function(self) self.d:SetTextColor(1,.7,.1) end
local onleave = function(self) self.d:SetTextColor(1,1,1) end

local ch_justify = function(self,num)
	local f = CreateFrame("Frame",nil,self)
	f:SetSize(80,24)
	f:SetPoint("TOP",0,-18)
	local d = M.setfont(f,12)
	d:SetPoint("BOTTOM",f,"TOP",0,2)
	d:SetText("Text Justify:")
	local m = M.setfont(f,17)
	m:SetPoint("TOP",0,-3)
	m:SetText(ct["justify_"..num])
	f.m = m; f.d = d; f.num = num
	f:EnableMouse(true)
	f:SetScript("OnEnter",onenter)
	f:SetScript("OnLeave",onleave)
	f:SetScript("OnMouseDown",on_push)
	self.just_con = f
end

local _f = function(self)
...
local StartConfigmode=function()
	if not InCombatLockdown()then
		for i=1,#ct.frames do
			...
			f:SetBackdropColor(.1,.1,.1,.4)
			f:SetBackdropBorderColor(.1,.1,.1)
			if not f.just_con then ch_justify(f,i);
			else f.just_con:Show() end
			f.fs = M.setfont(f,15,nil,nil,"CENTER","BOTTOM")
			f.fs:SetHeight(50)
			f.fs:SetPoint("BOTTOM",f,"TOP",0,0)
			if(i==1)then
				f.fs:SetText(DAMAGE)
				f.fs:SetTextColor(1,.1,.1,.8)
			elseif(i==2)then
			...
			if not(ct.scrollable)then
				f:SetScript("OnSizeChanged",_f)
			end

			f:SetScript("OnDragStop",f.StopMovingOrSizing)
			ct.locked=false
		end
		pr("unlocked. Type |cffff0000/xct lock|r to save or |cffff0000/rl|r to exit without saving.")
	else
...
SlashCmdList["XCT"]=function(input)
	...
	elseif(input=="reset")then
		...
		xCT4:SetHeight(172)
		xCT4:SetWidth(136)
		for i=1,4 do HissySavedPos["xCT"..i] = nil end
	else
		...
		pr("use |cffFF0000/xct reset|r to reset all positions.")
	end
end
...
if(ct.mergeaoespam)then
	if (ct.damage or ct.healing) then
		...
		do
			...
			local SQ=SQ
			local xCT4=xCT4
			M.set_updater(function(self, elapsed)
				local count
				local utime=time()
